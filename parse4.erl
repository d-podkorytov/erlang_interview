...
% user auth in URL does not supposts yet
-module(parse4).
-compile(export_all).
...
port(PATH_AND_OTHERS)when is_list(PATH_AND_OTHERS)-> {<<"80">>,list_to_binary(PATH_AND_OTHERS)}.
...
p(URL) when is_binary(URL) -> 
                               ...
                               try len(L) of
                                      ...
                                      AtArg -> #{arg_name(AtArg) => {error,{bad_parsing,URL}}} 
                                     catch
...
test24()->parse(<<"https://aa.com:$$bad_port$$/path/?var1=value1&var2=">>).
test25()->parse(<<"https://aa.com/path/?var1=value1&var2=">>).
