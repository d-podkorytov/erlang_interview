...
-define (URL_NO_ANYTHING, <<"">>).
-define (URL_ONLY_HOST, <<"aaaa.domain">>).
...
-define (RETURN_ERROR(Err1,Reason1,Stack1,X), 
                          {error,#{line     => ?LINE,error => Err1,reason => Reason1,stack => Stack1,
                                   url      => X,
                                   module   => ?MODULE, 
...
parse(URL) when is_binary(URL) -> %{Scheme,Authority_path_query_fragment} = scheme(URL,[]), % fix it! case of empty host and empty scheme here
             {Scheme,Authority_path_query_fragment} = try scheme(URL,[]) of
                                                         {Scheme1,Tail1} -> {Scheme1,Tail1};
                                                          EmptyHost      -> {list_to_binary(""),list_to_binary(lists:reverse(EmptyHost))} % host without scheme case
                                                       catch
                                                       Err1:Reason1:Stack1 -> ?RETURN_ERROR(Err1,Reason1,Stack1,{bad_scheme,URL})               
                                                       end,
             ...
             {Host,PortAndPathWithArgs}               = try host(Authority_path_query_fragment,[]) of
                                                               ...
                                                               RPortAndPathWithArgs -> Rev=lists:reverse(RPortAndPathWithArgs),
                                                                                       case host_and_path(list_to_binary(Rev),[]) of 
                                                                                       {Host1,Path1} -> {Host1,Path1};
                                                                                        OnlyHost when is_list(OnlyHost) -> {error,{only_host,URL}}
                                                                                       end
                                                              catch
                                                               Err:Reason:Stack -> ?RETURN_ERROR(Err,Reason,Stack,{bad_host,URL})
                                                                                   %io:format("~p:~p ~p ~n",[?MODULE,?LINE,{parsing_error,Err,Reason,Stack}]),
             ...
             {PortInt,Rest2}                             = try port(PortAndPathWithArgs,[]) of
                                                              ...
                                                              _:_:_ -> % port not found , set it default as 80
                                                                       PortAndPathWithArgsBin=list_to_binary(PortAndPathWithArgs),
                                                                       {80,<<"/",PortAndPathWithArgsBin/bitstring>>}  
                                                             end,
             {Path,Args}                                    = try raw_arguments(Rest2,[]) of
                                                               {R1,Args1} -> {R1,Args1};
                                                               L when is_list(L)        -> 
                                                                                           {Rest2,""}  % watch out here needs tests
                                                             catch
                                                              Err4:Reason4:Stack4-> ?RETURN_ERROR(Err4,Reason4,Stack4,{bad_arguments,URL})   
                                                             end,
...
test11()->parse(?URL_NO_ANYTHING).
test12()->parse(?URL_ONLY_HOST).
