...
%User auth form URL like https://clientjiberish:client/secretjiberish@api.example.com/users?username=tralala
...
user(RawUser) -> %{_Scheme,TailURL}=scheme(URL),
...
host(HOST_AND_OTHERS)when is_list(HOST_AND_OTHERS)-> 
              ...
              _ ->   
                     {error,{bad_host,?MODULE,?LINE,HOST_AND_OTHERS}} % bad cases
              end.
...
port({with_port,PORT_AND_OTHERS})when is_list(PORT_AND_OTHERS)-> 
              case string:str(PORT_AND_OTHERS,"/") of
               0   ->     try list_to_integer(PORT_AND_OTHERS) of
                           ...
                           _:_:_ -> {error,{bad_port,?MODULE,?LINE,PORT_AND_OTHERS}}   
                          end;
                          ...
                          lists:foldl(
                           fun(VarValue,Acc)->
                            try string:tokens(VarValue,"=") of % add try here
                              ...
                              [Var]         -> {error,{name_without_value,?MODULE,?LINE,Var}} 
                            catch
                             _:_:_ -> {error,{bad_arguments,?MODULE,?LINE,L}} 
                            end 
                           end,
                           #{},
                           string:tokens(L,"&")),
...
p(URL) when is_binary(URL) -> 
                               ...
                               try len(L) of
                                      ...
                                      AtArg -> #{arg_name(AtArg) => {error,{bad_parsing,?MODULE,?LINE,{n,AtArg},{l,L},URL}}} 
                                     catch
                                      _:_:_ -> #{error => {error ,{bad_parsing,?MODULE,?LINE,URL}}}  
                                     end. 
...
test_parse1()->parse(<<"https://aa.com:8888/path/?var1=value1&var2=">>).
test_parse2()->parse(<<"https://aa.com:$$bad_port$$/path/?var1=value1&var2=">>).
test_parse3()->parse(<<"https://aa.com:/path/?var1=value1">>).
test_parse4()->parse(<<"https://aa.com/path/?var1=value1">>).
...
%test_parse1()-> parse(<<"https://api.example.com/users?username=tralala">>).
%test_parse2()-> parse(<<"https://clientjiberish:client/secretjiberish@api.example.com/users?username=tralala">>).
%test_parse3()-> parse(<<"https://clientjiberish:client/secretjiberish@api.example.com:8888/users?username=tralala">>).

test_parse5()->parse("https://clientjiberish:client/secretjiberish@api.example.com:8888/users?username=tralala").
test_parse6()->parse("https://clientjiberish:client/secretjiberish@api.example.com/users?username=tralala").
test_parse7()->parse("https://api.example.com/users?var1=tralala&var2=value2").
test_parse8()->parse("https://api.example.com:8888/users?var1=tralala&var2=value2").
